...
public:
        ...
        if (this != &rhs) {
            if (rhs.size_ > data_.Capacity()) {
                ...
                if(rhs.size_ < size_){
                    ...
                    std::destroy_n(
                        data_.GetAddress()+rhs.size_, size_-rhs.size_
                    );
                }
                else {
                    std::copy(
    ...
    void Resize(size_t size_new) {
        if(size_new<size_){
            std::destroy_n(
                data_.GetAddress()+size_new, size_-size_new
            );
        }
        else {
            Reserve(size_new);
            std::uninitialized_value_construct_n(
                data_.GetAddress()+size_, size_new-size_
            );
        }
        size_ = size_new;
    }
    ...
    void PushBack(V&& value) {
        if (size_ == Capacity()) {
            ...
            new (tmp.GetAddress() + size_) T(std::forward<V>(value));
            if constexpr (
                std::is_nothrow_move_constructible_v<T> 
                || 
                !std::is_copy_constructible_v<T>
                ...
                std::uninitialized_move_n(
                    data_.GetAddress(), size_, tmp.GetAddress()
                );
            } else {
                std::uninitialized_copy_n(
                    data_.GetAddress(), size_, tmp.GetAddress()
            ...
            new (data_.GetAddress() + size_) T(std::forward<V>(value));
        }
        ++size_;
    }
    ...
    T& EmplaceBack(Ts&&... vs) {
        if (size_ == Capacity()) {
            RawMemory<T> tmp(size_ == 0 ? 1 : size_ * 2);
            new (tmp.GetAddress() + size_) T(std::forward<Ts>(vs)...);
            if constexpr (
                std::is_nothrow_move_constructible_v<T> 
                || 
                !std::is_copy_constructible_v<T>
